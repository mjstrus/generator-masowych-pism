...
def logout_button() -> None:
    """Przycisk wylogowania w sidebarze — bezpieczny klucz unikalny per strona."""
    import inspect
    frame = inspect.stack()[1]
    unique_key = f"logout_{frame.filename}_{frame.lineno}"
    if st.sidebar.button("Wyloguj", key=unique_key):
